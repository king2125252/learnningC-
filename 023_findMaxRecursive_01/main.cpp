...
int findMaxRecr1P(int* ary, int a, int b){
    if(a==b) return a;
    int max0 = findMaxRecr1P(ary, a+1, b);
    return ary[max0] > ary[a] ? max0 : a;
}

int findMaxRecr2P(int* ary, int a, int b){
    if(a==b) return b;
    int max0 = findMaxRecr2P(ary, a, b-1);
    return ary[max0] > ary[b] ? max0 : b;
}

int findMaxRecr3P(int* ary, int a, int b){
    if(a==b) return b;
    int m = (a+b)/2;
    int max0 = findMaxRecr3P(ary, a, m);
    int max1 = findMaxRecr3P(ary, m+1, b);
    return ary[max0] > ary[max1] ? max0 : max1;
}
...
int main()
{
    ...
    int max1P = findMaxRecr1P(score, a, b);
    int max2P = findMaxRecr2P(score, a, b);
    int max3P = findMaxRecr3P(score, a, b);

     cout << "max1p:" << max1P << endl;
     cout << "max2p:" << max2P << endl;
    cout << "max3p:" << max3P << endl;
}
